...
const CurrencySearch = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    await fetchLatestRate(currencyCode);
  };
      ...
      <form onSubmit={handleSubmit} className="mb-6">
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-grow">
            <label htmlFor="currency-code-latest" className="sr-only">
            ...
            {currencyCode.length > 0 && currencyCode.length < 3 && (
              <p className="text-xs text-gray-400 mt-1">
                Enter {3 - currencyCode.length} more character
                {3 - currencyCode.length !== 1 ? "s" : ""}
              </p>
            )}
          </div>
          <button
        ...
        <div className="space-y-4">
          <div className="p-5 bg-white rounded-lg border border-green-300 shadow-lg">
            ...
            <p className="text-sm text-gray-500">
              Last Refreshed:{" "}
              {new Date(rateData.lastRefreshed).toLocaleString()}
            </p>
            {isFromCache && (
